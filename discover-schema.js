...
function callback(err, schema) {
  ...
  */
  ...
  if (fs.existsSync(model_JSON_file)) {
    ...
    let bkp_file = path.join(
      output_directory,
      model_name + ".json" + ".bkp_" + now_ms
    );
    fs.renameSync(model_JSON_file, bkp_file);
    console.log("Backing up old JSON file..");
  }
  // write the new JSON file
  fs.writeFileSync(model_JSON_file, JSON.stringify(schema, null, 2));
  ...
  if (typeof model_config_obj[model_name] === "undefined") {
    ...
    fs.writeFileSync(
      model_config_file,
      JSON.stringify(model_config_obj, null, 2)
    );
  }
}
  ...
  switch (process.argv.length) {
    ...
    */
    case 6:
      ...
      var datasource_json = JSON.parse(
        fs.readFileSync(
      ...
      if (
        param11 === "-ds" &&
        param21 === "-sn" &&
        datasource_json.hasOwnProperty(datasource)
      ) {
        options = {};
